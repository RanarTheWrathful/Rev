 ...
 * @help Sun_0_GeneratorEx.js [version 2.7.0]
 ...
 * @default []
 * @type struct<ActorGenTvDataItem>[]
 *
 * @param genTvdList
...
SunMZ.Versions["Sun_0_GeneratorEx"] = "2.7.0";
...
ImageManager.loadGeneratorPic = function(filename) {
	return this.loadBitmap("img/generator_PIC/", filename);
};

ImageManager.loadGeneratorFg = function(filename) {
	return this.loadBitmap("img/generator_FG/", filename);
};

ImageManager.loadGeneratorTv = function(filename) {
	return this.loadBitmap("img/generator_TV/", filename);
};

ImageManager.loadGeneratorSv = function(filename) {
	return this.loadBitmap("img/generator_SV/", filename);
};
...
(() => {
	...
	SunMZ_GeneratorEx.genB64 = function(actor, mode, timeoutNum) {
		...
		for (var i = 0; i < SunMZ.GeneratorEx.VariableDataExList.length; i++) {
			...
			if (variableCodeData && variableCodeData.actorVarDataList.length > 0) {
				let actorVariableData = SunMZ_Core_Utils.findDataById(variableCodeData.actorVarDataList, actor._actorId);
				switch (mode) {
					case "PIC":
						if (actorVariableData && actorVariableData.genPicList.length > 0) {
							actorArmorDataFilter.push(...actorVariableData.genPicList);
						}
						break;
					case "FG":
						if (actorVariableData && actorVariableData.genFgList.length > 0) {
							actorArmorDataFilter.push(...actorVariableData.genFgList);
						}
						break;
					case "TV":
						if (actorVariableData && actorVariableData.genTvList.length > 0) {
							actorArmorDataFilter.push(...actorVariableData.genTvList);
						}
						break;
					case "TVD":
						if (actorVariableData && actorVariableData.genTvdList.length > 0) {
							actorArmorDataFilter.push(...actorVariableData.genTvdList);
						}
						break;
					case "SV":
						if (actorVariableData && actorVariableData.genSvList.length > 0) {
		...
		if (weaponId > 0) {
			let actorWeaponData = SunMZ_Core_Utils.findDataById(weapons[0]._actorWeaponDataList, actor._actorId);
			switch (mode) {
				case "PIC":
					if (actorWeaponData && actorWeaponData.genPicList.length > 0) {
						actorArmorDataFilter.push(...actorWeaponData.genPicList);
					}
					break;
				case "FG":
					if (actorWeaponData && actorWeaponData.genFgList.length > 0) {
						actorArmorDataFilter.push(...actorWeaponData.genFgList);
					}
					break;
				case "TV":
					if (actorWeaponData && actorWeaponData.genTvList.length > 0) {
						actorArmorDataFilter.push(...actorWeaponData.genTvList);
					}
					break;
				case "TVD":
					if (actorWeaponData && actorWeaponData.genTvdList.length > 0) {
						actorArmorDataFilter.push(...actorWeaponData.genTvdList);
					}
					break;
				case "SV":
					if (actorWeaponData && actorWeaponData.genSvList.length > 0) {
		...
		for (const armor of actor.armors().values()) {
			let actorArmorData = SunMZ_Core_Utils.findDataById(armor._actorArmorDataList, actor._actorId);
			switch (mode) {
				case "PIC":
					if (actorArmorData && actorArmorData.genPicList.length > 0) {
						actorArmorDataFilter.push(...actorArmorData.genPicList);
					}
					break;
				case "FG":
					if (actorArmorData && actorArmorData.genFgList.length > 0) {
						actorArmorDataFilter.push(...actorArmorData.genFgList);
					}
					break;
				case "TV":
					if (actorArmorData && actorArmorData.genTvList.length > 0) {
						actorArmorDataFilter.push(...actorArmorData.genTvList);
					}
					break;
				case "TVD":
					if (actorArmorData && actorArmorData.genTvdList.length > 0) {
						actorArmorDataFilter.push(...actorArmorData.genTvdList);
					}
					break;
				case "SV":
					if (actorArmorData && actorArmorData.genSvList.length > 0) {
		...
		switch (mode) {
			case "PIC":
				for (var i = 0; i < actor._generatorPicList.length; i++) {
					...
					let generatorBitmap = ImageManager.loadGeneratorPic(picImage);
			...
			case "FG":
				for (var i = 0; i < actor._generatorFgList.length; i++) {
					...
					let generatorBitmap = ImageManager.loadGeneratorFg(fgImage);
			...
			case "TV":
				for (var i = 0; i < actor._generatorTvList.length; i++) {
					...
					let generatorBitmap = ImageManager.loadGeneratorTv(tvImage);
			...
			case "TVD":
				for (var i = 0; i < actor._generatorTvdList.length; i++) {
					...
					let generatorBitmap = ImageManager.loadGeneratorTv(tvdImage);
			...
			case "SV":
				for (var i = 0; i < actor._generatorSvList.length; i++) {
					...
					let generatorBitmap = ImageManager.loadGeneratorSv(svImage);
	...
	Game_Actor.prototype.setup = function(actorId) {
		...
		//SunMZ_GeneratorEx.genB64(this, "TV", SunMZ.GeneratorEx.FaceTimeoutNum);
		
	};
	
	//修復存檔機制
	Game_Actor.prototype.SunMZ_GeneratorEx_RestorationData = function() {
		//this._generatorPicList = $dataActors[this._actorId]._generatorPicList || [];
		//this._generatorFgList = $dataActors[this._actorId]._generatorFgList || [];
		//this._generatorTvList = $dataActors[this._actorId]._generatorTvList || [];
		//this._generatorTvdList = $dataActors[this._actorId]._generatorTvdList || [];
		//this._generatorSvList = $dataActors[this._actorId]._generatorSvList || [];
		
		if (this._generatorPicList.length > 0 && JSON.stringify(this._generatorPicList).indexOf("@") > 0) {
		...
		if (this._generatorFgList.length > 0 && JSON.stringify(this._generatorFgList).indexOf("@") > 0) {
		...
		if (this._generatorTvList.length > 0 && JSON.stringify(this._generatorTvList).indexOf("@") > 0) {
		...
		if (this._generatorTvdList.length > 0 && JSON.stringify(this._generatorTvdList).indexOf("@") > 0) {
		...
		if (this._generatorSvList.length > 0 && JSON.stringify(this._generatorSvList).indexOf("@") > 0) {
	...
	/*
	const alias_SunMZ_GeneratorEx_Game_Party_setupStartingMembers = Game_Party.prototype.setupStartingMembers;
	Game_Party.prototype.setupStartingMembers = function() {
		alias_SunMZ_GeneratorEx_Game_Party_setupStartingMembers.call(this);
		
		for (let i = 1; i < $dataActors.length; i++) {
			if (!$dataSystem.partyMembers.includes(i)) {
				if ($gameActors.actor(i)) {
					this._standby.push(i);
				}
			}
		}
	};
	...
	Sprite_Character.prototype.setActor = function(acterType) {
		...
		switch (this._acterType) {
			...
			case "FOLLOWER":
				...
				this._actor = $gameParty.battleMembers()[this._character._memberIndex];
				break;
			case "EVENT":
	...
	Sprite_Character.prototype.setGeneratorBitmap = function() {
		...
		if (!this._isLoadedGeneratorInfo) {
			...
			for (var i = 0; i < SunMZ.GeneratorEx.VariableDataExList.length; i++) {
				...
				let variableCodeData = SunMZ_Core_Utils.findDataByCode(variableData.varCodeDataList, variableCode);
				if (variableCodeData && variableCodeData.actorVarDataList.length > 0) {
					let actorVariableData = SunMZ_Core_Utils.findDataById(variableCodeData.actorVarDataList, this._actor._actorId);
					switch (this._tvMode) {
						case "tv":
							if (actorVariableData && actorVariableData.genTvList.length > 0) {
								actorArmorDataFilter.push(...actorVariableData.genTvList);
			...
			if (weaponId > 0) {
				let actorWeaponData = SunMZ_Core_Utils.findDataById(weapons[0]._actorWeaponDataList, this._actor._actorId);
				switch (this._tvMode) {
					case "tv":
						if (actorWeaponData && actorWeaponData.genTvList.length > 0) {
							actorArmorDataFilter.push(...actorWeaponData.genTvList);
			...
			for (const armor of this._actor.armors().values()) {
				let actorArmorData = SunMZ_Core_Utils.findDataById(armor._actorArmorDataList, this._actor._actorId);
				switch (this._tvMode) {
					case "tv":
						if (actorArmorData && actorArmorData.genTvList.length > 0) {
							actorArmorDataFilter.push(...actorArmorData.genTvList);
	...
	Window_StatusBase.prototype.SunMZ_loadGeneratorExFaceImages = function(actor) {
		...
		for (var i = 0; i < SunMZ.GeneratorEx.VariableDataExList.length; i++) {
			...
			if (variableCodeData && variableCodeData.actorVarDataList.length > 0) {
				...
				if (actorVariableData && actorVariableData.genTvList.length > 0) {
					actorArmorDataFilter_tv.push(...actorVariableData.genTvList);
				}
			}
		}
		...
		if (weaponId > 0) {
			...
			if (actorWeaponData && actorWeaponData.genTvList.length > 0) {
				actorArmorDataFilter_tv.push(...actorWeaponData.genTvList);
			}
		}
		...
		for (const armor of actor.armors().values()) {
			...
			if (actorArmorData && actorArmorData.genTvList.length > 0) {
				actorArmorDataFilter_tv.push(...actorArmorData.genTvList);
			}
		}
